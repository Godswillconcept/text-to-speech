...
const Summarizer = ({ onSummarize, isLoading = false, error = '' }) => {
  ...
  const lengthOptions = [
    { id: 'short', name: 'Short', value: 0.3 },
    { id: 'medium', name: 'Medium', value: 0.5 },
    { id: 'long', name: 'Long', value: 0.7 },
  ];
  ...
  const handleSummarize = async () => {
    ...
    try {
      const selectedLength = lengthOptions.find(opt => opt.id === summaryLength)?.value || 0.5;
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-4">
            ...
            <div className="mt-1">
              <div
                ...
                className={`min-h-[200px] p-3 border ${
                  summary ? 'border-gray-300 bg-white' : 'border-gray-200 bg-gray-50'
                } rounded-md`}
              >
                {summary ? (
          ...
          {keyPoints.length > 0 && (
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-md">
              <h4 className="text-sm font-medium text-yellow-800 mb-2">Key Points</h4>
              <ul className="space-y-2">
                {keyPoints.map((point, index) => (
                  <li key={index} className="flex items-start">
                    <span className="flex-shrink-0 h-5 w-5 text-yellow-600">•</span>
                    <span className="text-sm text-yellow-700">{point}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          >
            {lengthOptions.map((option) => (
              <option key={option.id} value={option.id}>
                {option.name} (~{Math.round(option.value * 100)}% of original)
              </option>
            ))}
          </select>
        </div>
      ...
      <div className="bg-blue-50 border-l-4 border-blue-400 p-4 rounded-md mt-6">
        <div className="flex">
          ...
          <div className="ml-3">
            <h3 className="text-sm font-medium text-blue-800">Tips for better summaries</h3>
            <div className="mt-2 text-sm text-blue-700">
              <ul className="list-disc pl-5 space-y-1">
                <li>Longer texts generally produce better summaries</li>
                <li>Focus on one main topic per paragraph</li>
                <li>Use clear and concise language</li>
                <li>Try different summary lengths for varied results</li>
              </ul>
            </div>
          </div>
        </div>
